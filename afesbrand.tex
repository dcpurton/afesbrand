...
\lstdefinelanguage{expl3}{
  ...
  morekeywords={
    ...
    \AFESBrandIllustration, \AFESBrandFontSelect, \AFESBrandEmailSignature,
    \AFESBrandSetColours, \centering, \color, \huge, \colorbox, \textcolor,
    \raggedright, \quad, \node, \begin, \end
  },
  sensitive=true,
  morecomment=[l]\%
...
For functions that include other brand components, it's usually possible to
set the options for the inner function by adding it's prefix as well. For
example,

{\centering
 |\AFESBrandSetOptions{logo/variant=vertical, email/logo/variant=primary}|\par
}

will set the default logo variant to |vertical| for |\AFESBrandLogo|, but the
|primary| variant will be used in |\AFESBrandEmailSignature|.

Mostly fonts are fairly hard coded to comply with the style guidelines, so
defaults can only be set for inner functions occasionally.

These functions and environments can have inner functions options set:

\begin{itemize}
  \item \cs{AFESBrandEmailSignature}
  \item \cs{AFESBrandFrameDevice}
  \item |AFESBrandGridSystem|
  \item \cs{AFESBrandWaveEffect}
\end{itemize}
...
\begin{description}
  ...
  \item[padding] Set the padding around the logo. The following values are
    ...
    \begin{description}
      ...
      \item[{[north] [east] [south] [west]}] The logo is printed with padding
...
\cs_new_protected:Nn \__afesbrand_logo_<name>_<variant>:nnn
  {
    % padding values assume a default unit of cm
    % order of values is: upperx, uppery, lowerx, lowery
    \__afesbrand_padding_set:nn { centred }
      { 0.0 , 5.0 , 6.1925 , 0.0 }
    \__afesbrand_padding_set:nn { none }
      { 1.0605 , 3.9395 , 5.6019 , 0.5302 }
    % #1: some combination of north south east west separated by spaces
    % #2: colour for the background
    \__afesbrand_padding_draw:nn {#1} {#2}
    % path for logo
    % #3: colour of logo
    \path[fill=#3] ...;
  }
...
\begin{description}
  \item[family] Either |livvic| or |poetsenone|. If the \pkg{babel} package is
    loaded, these two families are defined with \cs{babelfont}.
...
\begin{description}
  ...
  \item[padding] Set the padding around the brand mark. The following values
    ...
    \begin{description}
      ...
      \item[{[north] [east] [south] [west]}] The brand mark is printed with
        padding on the specified sides.
    \end{description}
...
These options can also be set using \cs{AFESBrandSetOptions} and the prefix
|wave/|. Default settings for the font can be set using the prefix
|wave/font|.
...
can also be used to style the wave with or without the |wave/| and |font/| (or
|wave/font/|) prefixes. Note that where options clash such as |colour|, then
you need to specify |wave/colour|.

These options can also be set using \cs{AFESBrandSetOptions} and the prefix
|frame/|. Default settings for the wave and font can be set using the prefixes
|frame/wave/| and |frame/wave/font/|.
...
\begin{description}
  ...
  \item[padding] Set the padding around the image frame. The following values
    ...
    \begin{description}
      ...
      \item[{[north] [east] [south] [west]}] The image frame is printed with
        padding on the specified sides.
    \end{description}
...
\begin{function}{AFESBrandGridSystem}
  \begin{syntax}
    \oarg{options}
  \end{syntax}
  Print an AFES brand grid system with the specified \meta{options}.
...
\begin{figure}
  \begin{lstlisting}
\colorbox{neutral}{%
  \begin{AFESBrandGridSystem}[body/colour=hero,
      head/colour=white]
    \AFESBrandFontHeadOne\color{accent}COMMS\\
    \textcolor{white}{TITLE GOES}\\
    HERE
  \end{AFESBrandGridSystem}%
}
  \end{lstlisting}
...
\begin{description}
  ...
  \item[body/height] The height of the body. Default: \emph{Natural height of
    content}
  \item[head] A \meta{boolean} value specifying whether to include the header.
    Default: |true|
  ...
  \item[sidebar/width] The width of the |sidebar| unless less the |url| is
    shown and wider than the specified width. If |0pt| then the natural
    width of the |url| is used. Default: 0pt
...
The options supported by \cs{AFESBrandLogo} can also be used to style the logo
with or without the |logo/| prefix.

These options can also be set using \cs{AFESBrandSetOptions} and the prefix
|grid/|. Default settings for the logo can be set using the prefix
|grid/logo/|.
...
\begin{description}
  ...
  \item[logo/sep] Set the space between the logo and the signature text.
    Default: |1em|
  ...
  \item[web] Set the web site URL. Default: |afes.org.au|
...
The options supported by \cs{AFESBrandLogo} can also be used to style the logo
with the |logo/| prefix. Note that where options clash such as |height|, then
you need to specify |logo/height|.

These options can also be set using \cs{AFESBrandSetOptions} and the prefix
|email/|. Default settings for the logo can be set using the prefix
|email/logo/|.
